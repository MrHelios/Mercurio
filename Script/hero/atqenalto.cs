...
public class atqenalto : atqDistancia
{
    void Start()
    {
        anim = GetComponent<Animator>();
        nombre_anim = "atq_alto";
    ...
    public override void efecto()
    {
        if (disp != null)
        {
            ...
            {
                nuevo.transform.rotation = Quaternion.Euler(0, 0, 45);
                nuevo.GetComponent<Rigidbody2D>().velocity = new Vector2(velocidad, velocidad);
            }
            else
            {
                nuevo.transform.rotation = Quaternion.Euler(0, 0, -45);
